...
<sch:schema xmlns:sch="http://www.ascc.net/xml/schematron" 
  ...
  <sch:title>Rules for CSW-2.0.1 GetRecords response entities.</sch:title>
  ...
  <sch:phase id="OneToTenAdHocRecordsPhase">
    ...
    <sch:active pattern="OneOrMoreHitsPattern"/>
    <sch:active pattern="OneToTenRecordsPattern"/>
    <sch:active pattern="IdTypeModifiedElementsPattern"/>
  </sch:phase>
  
  <sch:phase id="IdTypeModifiedElementsPhase">
    <sch:active pattern="GetRecordsResponsePattern"/>
    <sch:active pattern="TenRecordsPattern"/>
  ...
  <sch:phase id="BoxModifiedElementsPhase">
    <sch:active pattern="GetRecordsResponsePattern"/>
    <sch:active pattern="OneToTenRecordsPattern"/>
    <sch:active pattern="BoxModifiedElementsPattern"/>
  </sch:phase>
  ...
  <sch:pattern id="GetRecordsResponsePattern" name="GetRecordsResponsePattern">
	...
	The document element must have [local name] = "GetRecordsResponse" and [namespace name] = "http://www.opengis.net/cat/csw".
      </sch:assert>
       <sch:assert id="status" 
        test="csw:GetRecordsResponse/csw:SearchStatus/@status='complete'">
	The csw:SearchStatus/@status attribute must have the value "complete".
      </sch:assert>
    </sch:rule>
  </sch:pattern>
  ...
  <sch:pattern id="IdTypeModifiedElementsPattern" name="IdTypeModifiedElementsPattern">
    <sch:p xml:lang="en">Checks that csw:Record instances contain only the following three child elements: dc:identifier, dc:type, dct:modified</sch:p>
    <sch:rule id="IdTypeModifiedElements" context="/csw:GetRecordsResponse/csw:SearchResults/csw:Record">
	...
	The csw:Record elements must have three child elements.
      ...
      <sch:assert id="dct.modified" 
        test="dct:modified">Missing dct:modified element.</sch:assert>
    </sch:rule>
  </sch:pattern>
  ...
  <sch:pattern id="BoxModifiedElementsPattern" name="BoxModifiedElementsPattern">
    <sch:p xml:lang="en">Checks that included records contain the following elements: dct:modified, ows:BoundingBox.</sch:p>
    <sch:rule id="BoxModifiedElements" context="/csw:GetRecordsResponse/csw:SearchResults/*">
      <sch:assert id="BoxModifiedElements.modified" 
        test="dct:modified">Missing dct:modified element.</sch:assert>
      <sch:assert id="BoxModifiedElements.box" 
